...
export class MyPromise<T> implements IMyPromise<T> {
  ...
  private run() {
    ...
    while (this.handlers.length) {
      const { onfulfilled, onrejected, resolve, reject } =
        this.handlers.shift() as IHandler
  ...
  public then<TR, TF = never>(
    onfulfilled?: OnFulFilledType<T, TR>,
    onrejected?: OnRejectedType<TF>,
  ) {
  ...
  static all<T extends readonly unknown[] | []>(
    values: T,
  ): MyPromise<AllResult<T>> {
    return new MyPromise((resolve, reject) => {
      const result: Awaited<T[number]>[] = []
      let count = 0
      for (let i = 0, len = values.length; i < len; i++) {
        ;(MyPromise.resolve(values[i]) as MyPromise<Awaited<T[number]>>).then(
          res => {
            result[i] = res
            count++
